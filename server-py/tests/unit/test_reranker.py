...
from app.services.rag.reranker import CrossEncoderReranker, RERANK_THRESHOLD


# ── 构造辅助函数 ──────────────────────────────────────────────

def _make_doc(content: str, title: str = "测试文档", doc_id: str = "doc-001",
              category: str = "通用", vector_score: float = 0.8) -> Document:
...
def test_reranker_should_include_metadata_in_results():
    ...
    reranker = MockReranker(threshold=0.0)
    docs = [_make_doc("年假测试内容", title="制度文档", doc_id="abc-123",
                      category="HR制度", vector_score=0.95)]
