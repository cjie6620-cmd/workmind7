...
class TestLiveRagasSingleMetric:
    ...
    async def test_faithfulness_with_real_llm(
        self, evaluator_llm, sampled_golden_dataset, eval_thresholds
    ):
        ...
        assert avg >= threshold, (
            f"Faithfulness = {avg:.3f} < {threshold}"
        )

    async def test_context_recall_with_real_llm(
        self, evaluator_llm, sampled_golden_dataset, eval_thresholds
    ):
        ...
        assert avg >= threshold, (
            f"ContextRecall = {avg:.3f} < {threshold}"
        )

    async def test_factual_correctness_with_real_llm(
        self, evaluator_llm, sampled_golden_dataset, eval_thresholds
    ):
        ...
        assert avg >= threshold, (
            f"FactualCorrectness = {avg:.3f} < {threshold}"
        )
...
class TestLiveRagasBatchEvaluation:
    ...
    async def test_batch_evaluate_with_ragas(
        self, evaluator_llm, sampled_golden_dataset, eval_thresholds
    ):
        """
        批量评测：三个核心指标应全部达标
...
class TestLiveRagasBadCaseAnalysis:
    ...
    async def test_identify_faithfulness_bad_cases(
        self, evaluator_llm, sampled_golden_dataset
    ):
    ...
    async def test_identify_context_recall_bad_cases(
        self, evaluator_llm, sampled_golden_dataset
    ):
