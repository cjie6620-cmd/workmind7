...
class TestLiveRetrievalQuality:
    ...
    async def test_real_retrieval_precision_at_4(
        ...
        assert avg >= threshold, (
            f"Precision@4 = {avg:.3f} < {threshold}"
        )

    async def test_real_retrieval_recall_at_4(
        self, single_hop_queries, eval_thresholds
    ):
...
class TestLiveEndToEndGeneration:
    ...
    async def test_e2e_rag_with_real_services(
