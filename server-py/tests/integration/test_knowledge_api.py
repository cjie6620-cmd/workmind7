...
async def test_should_upload_text_content(app_client):
    ...
    with patch(
        "app.routes.knowledge.ingest_document",
    ...
    assert ingest.await_args.kwargs["owner_user_id"] == "dev"
...
async def test_should_list_documents(app_client):
    """文档列表应返回 200"""
    with patch(
        "app.routes.knowledge.get_doc_registry",
        new_callable=AsyncMock,
        return_value=[
...
async def test_should_list_documents_by_category(app_client):
    ...
    with patch("app.routes.knowledge.get_doc_registry", new_callable=AsyncMock, return_value=docs):
...
async def test_should_delete_document(app_client):
    """删除文档应返回 200"""
    with patch("app.routes.knowledge.delete_document", new_callable=AsyncMock) as delete:
        response = await app_client.delete("/api/knowledge/documents/test-doc-id")

    assert response.status_code == 200
    assert response.json()["success"] is True
    assert delete.await_args.kwargs == {"requester_user_id": "dev", "is_admin": True}
...
async def test_should_return_404_for_nonexistent_document(app_client):
    """删除不存在的文档应返回 404"""
    with patch("app.routes.knowledge.delete_document", new_callable=AsyncMock, side_effect=ValueError("文档不存在")):
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_should_return_403_when_document_delete_is_forbidden(app_client):
    with patch(
        "app.routes.knowledge.delete_document",
        new_callable=AsyncMock,
        side_effect=PermissionError("无权删除该文档"),
    ):
        response = await app_client.delete("/api/knowledge/documents/other-user-doc")
