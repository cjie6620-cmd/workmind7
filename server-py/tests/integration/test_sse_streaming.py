...
async def test_should_stream_sse_events_in_correct_order(app_client):
    ...
    with (
        patch("app.routes.knowledge.rag_query_stream", new_callable=AsyncMock, return_value=mock_query_result),
        patch("app.routes.knowledge.save_message", new_callable=AsyncMock),
        patch("app.routes.knowledge.assert_session_owner", new_callable=AsyncMock),
    ):
...
async def test_should_return_done_with_session_id(app_client):
    ...
    with (
        ...
        response = await app_client.post(
            "/api/knowledge/query/stream",
            json={"question": "测试问题", "sessionId": "knowledge_dev_test123"},
        )
    ...
    assert done_event["data"]["sessionId"] == "knowledge_dev_test123"
