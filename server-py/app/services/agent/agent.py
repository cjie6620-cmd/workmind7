...
async def run_agent(task, emit_event):
    ...
    try:
        async for msg, metadata in agent_graph.astream(
            ...
            if node == 'agent' and hasattr(msg, 'tool_call_chunks'):
                for tc in (msg.tool_call_chunks or []):
                    if tc.get('name') and tc.get('id') and tc['id'] not in pending_tool_calls:
                ...
                # 流式 token（排除 tool_call chunk）
                if msg.content and not (msg.tool_call_chunks or []):
        ...
        await emit_event('done', {'steps': step_count, 'finalAnswer': final_answer})
