...
async def list_configs(config_type: str) -> list[dict]:
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            select(AgentConfig)
            .where(AgentConfig.config_type == config_type)
...
async def update_config(config_id: str, *, name: str = None, config_json: dict = None) -> dict:
    """
    更新配置
...
async def delete_config(config_id: str) -> bool:
    ...
    async with async_session_factory() as session:
        ...
        await session.execute(
            sa_delete(AgentConfig).where(AgentConfig.id == config_id)
        )
        await session.commit()
...
async def seed_if_empty(config_type: str, seeds: list[dict]) -> int:
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            select(AgentConfig).where(AgentConfig.config_type == config_type)
        )
