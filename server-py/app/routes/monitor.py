...
持久化策略：
- /stats 使用 PostgreSQL 对指定日和最近 7 日做有界聚合
- 内存 _calls 仅保留最近记录，供数据库异常时显式降级
...
async def _query_stats_from_db(selected_day: date) -> dict:
    ...
    today_stmt = select(
        ...
        func.coalesce(
            func.percentile_cont(0.5).within_group(latency_value),
            0.0,
        ).label("p50"),
        func.coalesce(
            func.percentile_cont(0.9).within_group(latency_value),
            0.0,
        ).label("p90"),
        func.coalesce(
            func.percentile_cont(0.99).within_group(latency_value),
            0.0,
        ).label("p99"),
    ...
    # time 是 UTC-naive；先解释为 UTC，再转换到业务时区后按本地日期分组。
    business_local_time = func.timezone(
        business_timezone_name(),
        func.timezone("UTC", MonitorRecord.time),
    )
    day_expression = cast(business_local_time, SQLDate)
    last7_stmt = (
        select(
    ...
    feature_stmt = (
        select(
            MonitorRecord.feature.label("feature"),
            ...
            func.coalesce(
                func.sum(func.coalesce(MonitorRecord.input_tokens, 0) + func.coalesce(MonitorRecord.output_tokens, 0)),
                0,
            ).label("tokens"),
        )
        .where(
    ...
    recent_stmt = (
        select(
            MonitorRecord.time.label("time"),
            MonitorRecord.feature.label("feature"),
            MonitorRecord.input_tokens.label("input_tokens"),
            MonitorRecord.output_tokens.label("output_tokens"),
            MonitorRecord.cost_cny.label("cost_cny"),
            MonitorRecord.latency_ms.label("latency_ms"),
            MonitorRecord.from_cache.label("from_cache"),
            MonitorRecord.error.label("error"),
        )
