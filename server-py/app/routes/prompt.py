...
from ..services.pricing import calculate_token_cost
...
# A/B 后台任务强引用：断连只停推送，不取消评分
_ab_tasks: set[asyncio.Task] = set()


@prompt_router.post("/test/stream")
async def prompt_test_stream(req: PromptTestRequest, request: Request):
    """
    测试 Prompt 接口
    ...
    async def event_generator():
        try:
            # 创建独立模型实例用于测试
            test_model = create_chat_model(temperature=temperature, streaming=True, max_tokens=max_tokens)
            ...
            async for chunk in test_model.astream(messages):
                if not await client_still_connected(request):
                    break
                if chunk.content:
...
async def prompt_ab_test_stream(req: PromptAbTestRequest, request: Request):
    """
    A/B 测试接口（SSE 流式）
    ...
    model_a = create_chat_model(temperature=temperature, streaming=True, max_tokens=max_tokens)
    model_b = create_chat_model(temperature=temperature, streaming=True, max_tokens=max_tokens)
        ...
        try:
            ...
            await queue.put(
                sse_event(
                    f"done_{side}",
                    {
                        "latencyMs": round((_time.perf_counter() - start) * 1000),
                        "inputTokens": input_tokens,
                        "outputTokens": output_tokens,
                    },
                )
    ...
    async def run():
        ...
        try:
            ...
            if answer_a and answer_b:
                ...
                raise RuntimeError("A/B 测试至少一侧生成失败")
        except Exception as err:
            failed = True
    ...
    run_task = asyncio.create_task(run(), name="prompt-ab-test")
    _ab_tasks.add(run_task)
    run_task.add_done_callback(_ab_tasks.discard)

    async def event_generator():
        yield sse_event("start", {"maxTokens": max_tokens})
